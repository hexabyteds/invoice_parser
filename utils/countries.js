// Country -> dial code reference data, shared by signup and profile-edit
// validation. Mirrored in frontend/src/constants/countries.js so the
// dropdown and the backend check agree on both the country list and the
// dial code for each entry; keep the two files in sync if this list changes.
const COUNTRIES = [
  ...
  { name: "Congo (Congo-Brazzaville)", dialCode: "+242" },
...
// Confirms the country exists in the reference list and that the supplied
// dial code is exactly the one associated with it — rejects both an
// unknown country and a country/code combination that don't match (e.g.
// country "Pakistan" paired with code "+971"), rather than trusting
// whatever the client sent.
function validateCountryAndCode(country, countryCode) {
  ...
  if (match.dialCode !== countryCode.trim()) {
    throw new Error("Country code does not match the selected country.");
  }
