...
VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    ...
    async findByCompanyName(userId, companyName, excludeId = null) {
        ...
        const sql = excludeId
            ? `SELECT id FROM suppliers WHERE user_id = ? AND LOWER(company_name) = LOWER(?) AND id != ? LIMIT 1`
            : `SELECT id FROM suppliers WHERE user_id = ? AND LOWER(company_name) = LOWER(?) LIMIT 1`;
    ...
    async update(id, userId, supplier) {
    ...
    async updateStatus(id, userId, status) {
        ...
        const [result] = await db.execute(
            `UPDATE suppliers SET status=? WHERE id=? AND user_id=?`,
            [status, id, userId]
        );
